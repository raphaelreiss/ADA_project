...
In order to get better insight into the households in our dataset, we decided to group them using k-means clustering. We computed the number of times each household has ever bought a specific item and then we obtained 300 features acusing matrix decomposition with SVD and then we clustered the household according to these features. We applied TF-IDF normalisation in order to reduce the importance of the articles bought frequently by almost every household.
...
<img src="../src/kmeans_nb_of_clusters.png" alt="Optimal_number_k_" style="zoom:60%;" />
...
<img src="../src/3d_scatter_plot_of_groups.png" alt="Optimal_number_k_" style="zoom:72%;" />In order to get a more detailed view, we use the scatter plot matrix. We indeed see that the three first categories explain most of the variance because the groups are clearly separated along these axis. The gasoline group, in red is clearly separated from all others. This is a particularity of the shopping habbits of these households. Indeed, as it was shown before, gasoline is the highest bought item by sales value by far.
